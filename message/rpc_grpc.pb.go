...
type ProcessorClient interface {
	Deploy(ctx context.Context, opts ...grpc.CallOption) (Processor_DeployClient, error)
...
func (c *processorClient) Deploy(ctx context.Context, opts ...grpc.CallOption) (Processor_DeployClient, error) {
	stream, err := c.cc.NewStream(ctx, &Processor_ServiceDesc.Streams[0], "/message.Processor/Deploy", opts...)
	if err != nil {
		return nil, err
	}
	x := &processorDeployClient{stream}
	return x, nil
}

type Processor_DeployClient interface {
	Send(*DeployRequest) error
	CloseAndRecv() (*Empty, error)
	grpc.ClientStream
}

type processorDeployClient struct {
	grpc.ClientStream
}

func (x *processorDeployClient) Send(m *DeployRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *processorDeployClient) CloseAndRecv() (*Empty, error) {
...
func (c *processorClient) Process(ctx context.Context, in *ProcessRequest, opts ...grpc.CallOption) (*ProcessResponse, error) {
	...
	err := c.cc.Invoke(ctx, "/message.Processor/Process", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type ProcessorServer interface {
	Deploy(Processor_DeployServer) error
...
func (UnimplementedProcessorServer) Deploy(Processor_DeployServer) error {
...
func _Processor_Deploy_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(ProcessorServer).Deploy(&processorDeployServer{stream})
}

type Processor_DeployServer interface {
	SendAndClose(*Empty) error
	Recv() (*DeployRequest, error)
	grpc.ServerStream
}

type processorDeployServer struct {
	grpc.ServerStream
}

func (x *processorDeployServer) SendAndClose(m *Empty) error {
	return x.ServerStream.SendMsg(m)
}

func (x *processorDeployServer) Recv() (*DeployRequest, error) {
	m := new(DeployRequest)
...
func _Processor_Process_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	...
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/message.Processor/Process",
	}
...
var Processor_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "Deploy",
			Handler:       _Processor_Deploy_Handler,
			ClientStreams: true,
		},
	},
